...
function onCreatePost()
	...
	for i = 1,4 do makeLuaSprite(assets[i], ''..assets[i], xpos[i], ypos[i]); if i < 4 then addLuaSprite(assets[i]) else addLuaSprite(assets[i], true) end; end
        ...
        scaleObject('dreamfrontwall', 4, 4);
        scaleObject('dreammidwall', 4, 4);
        scaleObject('dreamstairs', 4, 4);

	makeAnimatedLuaSprite('waves', 'waves', -200, -100); addAnimationByPrefix('waves', 'idle', 'idle', 6, true); setObjectCamera('waves', 'camGame'); setProperty('waves.alpha', 0); scaleObject('waves', 1420/512, 920/512); addLuaSprite('waves', true);
